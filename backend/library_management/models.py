...
class User_Book(models.Model):
    ...
    status = models.CharField(
        max_length=20, choices=BorrowStatus.choices, default=BorrowStatus.PENDING
    )
    note = models.CharField(max_length=255, null=True)

    def set_due_date(self, borrowing_days):
        valid_statuses = [User_Book.BorrowStatus.BORROWING]
...
class User_Book_Detail_Fine(models.Model):
    user_book = models.OneToOneField(
        User_Book, on_delete=models.CASCADE, primary_key=True, related_name="detail"
    )
    ...
    def total_fine(self):
        ...
        return (
            self.late_dates
            * self.setting.borrowing_overdue_fine
            * self.user_book.borrowing_quantity
        )
