...
class UserViewSet(BaseViewSet):
    ...
    @action(
        methods=["GET"],
        url_path="current_user/borrowing_list",
        detail=False,
    )
    def borrowing_list(self, request):
        ...
        if status_param:
            borrowing = searching_services.search_item_by_status(
                borrowing, status_param, User_Book.BorrowStatus
            )
        ...
        return Response(
            BorrowSerializer(borrowing, many=True).data, status=status.HTTP_200_OK
        )
    ...
    def orders_list(self, request):
        ...
        if page is not None:
            ...
            return p.get_paginated_response(serializer.data)
        return Response(
            ReservationSerializer(orders, many=True).data, status=status.HTTP_200_OK
        )
...
class BorrowViewSet(
    ...
    def get_queryset(self):
        ...
        if book:
            queryset = searching_services.search_item_name(
                queryset, book, field="book__name"
            )
        if status:
            queryset = searching_services.search_item_by_status(
                queryset, status, User_Book.BorrowStatus
            )
        return queryset
...
class ReservationViewSet(
    ...
    def get_queryset(self):
        ...
        if status:
            queryset = searching_services.search_item_by_status(
                queryset, status, Reservation.ReservationStatus
            )
        return queryset
    ...
    def update_status(self, request, pk=None):
        ...
        new_status = request.data.get("status")
        try:
            reservation_obj = reservation_services.update_reservation_status(
                reservation_obj, new_status
            )
        except ValueError as e:
            return Response({"message": str(e)}, status=status.HTTP_400_BAD_REQUEST)
