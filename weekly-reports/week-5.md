# BÁO CÁO TUẦN 5
## TUẦN: 5
## NHÓM: 10
## ĐỀ TÀI: HỆ THỐNG QUẢN LÝ THƯ VIỆN
## NHÓM TRƯỞNG: TRẦN KHÁNH AN - 2351050004

## 1. CÔNG VIỆC ĐÃ HOÀN THÀNH
| Thành viên             | MSSV       | Công việc                                    |
|------------------------|------------|----------------------------------------------|
| Trần Khánh An          | 2351050004 | Thiết kế thêm api + phân quyền               |
| Phùng Thị Kim Ngân     | 2354050073 | Tìm thêm dữ liệu test và dữ liệu cho db      |

## 2. Tiến độ tổng thể

| Hạng mục              | Trạng thái   |   %   |
|-----------------------|--------------|-------|
| Phân tích yêu cầu     | Hoàn thành   | 100%  |
| Thiết kế kiến trúc    | Đang làm     | 100%  |
|(Theo yêu cầu)         |              |       |
| Backend API           | Đang làm     | 70%   |
| Frontend UI           | Đang làm     | 70%   |
| Docker                | Chưa bắt đầu |  0%   |
| Testing               | Chưa bắt đầu |  0%   | 

**Tổng tiến độ: *43%*


## 3. Kế hoạch tuần tới

| Thành viên             |        Công việc dự kiến                        |
|------------------------|-------------------------------------------------|
| Trần Khánh An          |  Hoàn thành UI + Backend + unit test            |
| Phùng Thị Kim Ngân     |  unit test + integration test + test controller |


## 4. Khó khăn / Cần hỗ trợ

- [ ] [Mô tả khó khăn nếu có]  
- [ ] [Câu hỏi cần GV giải đáp nếu có]  

---

*Ngày nộp: 5/4/2026*  
*Xác nhận của Nhóm trưởng: [Trần Khánh An]*